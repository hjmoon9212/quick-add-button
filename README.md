# Quick Add Button

노트 본문에 **버튼**을 놓고, 누르면 미니 폼으로 **다른 노트의 특정 헤딩 아래**에
할일 한 줄을 넣는 Obsidian 플러그인.

QuickAdd 의 Capture 로 하던 "임시 할일 추가"를 확장한 것이다. QuickAdd 는 파일 맨 위나 맨
아래로만 넣을 수 있는데, 여기서는 **파일과 헤딩까지 설정에서 정해** 그 헤딩 아래로
넣는다.

## 쓰는 법

노트 아무 데나 이렇게 넣는다.

````markdown
```quick-add-button
```
````

빈 블록이면 켜져 있는 규칙이 전부 버튼으로 뜬다. 골라 쓰려면:

````markdown
```quick-add-button
rules: TempTask, Idea
```

```quick-add-button
exclude: Maint
```
````

코드블록을 놓은 노트에 가 있지 않아도 **왼쪽 리본 아이콘**으로 부를 수 있다.
누르면 버튼 목록이 뜨고, 고르면 그 버튼을 누른 것과 똑같이 폼이 뜬다.
이름·라벨뿐 아니라 넣을 자리(파일 › 헤딩)로도 검색된다. 아이콘을 숨기고 싶으면
옵시디언 기본 기능(리본 우클릭 → 관리)을 쓴다.

목록에 무엇을 넣을지는 **규칙마다 「리본 목록에 넣기」로 고른다.** 노트에 박아 둔
버튼은 그 노트에서 누르면 되므로 리본까지 차지할 이유가 없다. 여기서 빼도
코드블록과 커맨드로는 그대로 쓴다 — 규칙을 끄는 것과 다르다.

버튼을 누르면:

```
┌─ ⚡ 임시 할일 ──────────────────────┐
│ →  Temp Tasks › Quick Add          │
│ [__________________________]       │ ← Enter = 추가
│ 태그      [#task ▾]                │
│ GCal      [#gcal/work ▾]           │ ← 규칙에 캘린더가 있을 때만
│ 🛫 시작   [____]                    │
│ 📅 마감   [____]                    │
│           [오늘][내일][+7일][지우기] │
│ 우선순위  [없음 ▾]                  │
│ 위치      [목록 끝 ▾]               │
│           ☑ ➕ 생성일  ☑ 🆔 아이디  │
│           ☐ ↪ 추가 후 이동           │
│ 미리보기  - [ ] #task … 📅 …        │
│                            [추가]  │
└────────────────────────────────────┘
```

들어가는 줄은 Obsidian Tasks 문법 그대로다.

```
- [ ] #task 내용 ⏫ 🆔 abc123 ➕ 2026-08-07 🛫 2026-08-08 📅 2026-08-14
```

GCal 캘린더를 지정한 버튼이면 태그 뒤에 라우팅 태그가 붙는다.

```
- [ ] #task/tobuy #gcal/routine 내용 📅 2026-08-14
```

연달아 넣는 경우가 많아 추가해도 폼이 닫히지 않는다 (Esc 로 닫는다).

**↪ 추가 후 이동**을 켜면 폼은 그대로 둔 채 **뒤에 있는 노트가 넣은 줄로 옮겨간다.**
넣는 족족 반영되는 게 보이고, 폼을 닫으면 바로 그 자리다. 포커스는 입력칸에 남으므로
연속 추가도 그대로 된다. 기본값은 규칙마다 설정에서 정하고 폼에서 그때그때 바꾼다.

**이미 있는 것을 먼저 쓴다** — 새 탭을 함부로 만들지 않는다.

| 그 노트가 | 어떻게 |
|---|---|
| 지금 보고 있는 노트 | 다시 열지 않고 **스크롤만**. 편집 중이던 상태가 안 튄다 |
| 다른 탭에 열려 있음 | **그 탭을 앞으로.** 사이드바보다 본문 영역을 먼저 고른다 |
| 어디에도 없음 | 지금 탭에 연다 |

어느 경우에도 **포커스는 폼에 남는다.** 탭이 포커스를 가져가면 입력칸에서 커서가
빠져 연속 추가가 끊긴다.

## 설정

**설정 → Quick Add Button** 에서 규칙(= 버튼)을 추가·편집·정렬·삭제한다.
규칙 하나가 **넣을 자리 하나**를 가리킨다.

| 항목 | |
|---|---|
| **파일** | 입력하면 볼트의 노트를 검색해 고른다. 고정 경로 말고 **버튼을 누른 노트 기준**으로도 적을 수 있다 (아래) |
| **헤딩** | 고른 파일의 헤딩 목록이 바로 채워진다. **이 헤딩 아래**에 할일이 들어간다 |
| **태그** | 이 버튼이 쓸 태그. 콤마로 여러 개 두면 폼에서 고른다 (`#task, #task/ISSUE`). 맨 앞이 기본값 |
| **GCal 캘린더** | [tasks-gcal-sync](https://github.com/hjmoon9212/tasks-gcal-sync) 의 라우팅 태그 (`#gcal/routine`). 붙이면 그 캘린더로 일정이 간다. 비워 두면 폼에 칸이 안 뜨고 줄에도 안 붙는다. 콤마로 여러 개 두면 폼에서 고르고, 폼에는 `없음` 도 있다. **전체** 버튼으로 아는 캘린더를 한 번에 넣고, **지우기** 로 비운다 |

### 파일 칸에 적을 수 있는 것

| 적는 값 | 어디로 |
|---|---|
| `0. Note/0. Inbox/Temp Tasks.md` | 볼트 절대경로. 어디서 눌러도 이 노트 |
| `@current` | **버튼을 누른 노트 자신.** 데일리 노트마다 그 노트에 넣을 때 |
| `./로그.md` | 누른 노트와 **같은 폴더** |
| `../Living.md` | 누른 노트의 **상위 폴더** |
| `Temp Tasks` | 이름만. `[[Temp Tasks]]` 와 똑같이 해석한다 |

"누른 노트"는 코드블록의 버튼이면 **그 블록이 있는 노트**, 리본·커맨드로 불렀으면
**그때 보고 있던 노트**다. 분할 화면에서도 블록이 놓인 쪽이 기준이라 옆 창에
잘못 들어가지 않는다.

`@current` · `./` · `../` 는 누를 때마다 대상이 달라지므로 헤딩 목록을 미리 만들 수
없다. 그때는 헤딩 **이름을 직접 적고 단계를 고른다.**

| 단계 | 뜻 |
|---|---|
| `아무 단계` | 이름만 맞으면 된다. 노트마다 `# 할 일` 이기도 `## 할 일` 이기도 할 때 |
| `#` ~ `######` | 그 단계의 헤딩만. 같은 이름이 여러 단계에 있을 때 |

그 헤딩이 없는 노트에서 누르면 아무것도 넣지 않고 알린다.

헤딩 아래 어디에 넣을지는 `목록 끝` / `헤딩 바로 밑` 중에 고른다.
`↪ 추가 후 그 노트로 이동` 토글로 넣은 뒤 그 줄로 갈지 정한다.

규칙이 가리키는 파일이나 헤딩이 사라지면 그 규칙 줄에 `⚠️` 가 뜬다. 저장을 막지는
않는다 — 파일을 나중에 만들 수도 있으니까.

규칙 **삭제**와 **JSON 적용**(설정 전체 교체)은 확인창을 한 번 거친다. 기본 포커스가
`취소` 에 있으므로 Enter 를 눌러도 실행되지 않는다.

규칙마다 커맨드가 등록되므로 커맨드 팔레트와 모바일 툴바에서도 부를 수 있다.
리본 아이콘은 규칙을 고르는 목록을 띄우고, 커맨드는 그 규칙의 폼을 바로 연다.

## 동작 세부

- **목록 끝** = 고른 헤딩 **직속 본문**의 마지막 목록 항목 뒤. 목록 다음에 메모가 있어도
  메모 뒤가 아니라 목록에 붙고, 목록 중간에 빈 줄이 있어도 끝까지 간다. 목록이 아직
  없으면 본문 끝(다음 헤딩 앞)에 넣는다
- 하위 헤딩은 **범위 밖**이다. `# Tasks` 를 골랐으면 그 아래 `## 완료` 안으로 들어가지 않는다
- 헤딩을 못 찾으면 아무것도 고치지 않고 알린다 (엉뚱한 자리에 넣지 않는다)
- **코드펜스 안의 `#` 은 헤딩으로 보지 않는다** — 펜스 마커의 종류와 길이를 지켜보므로
  4-백틱 블록 안의 3-백틱이나 `~~~` 혼용에도 뒤집히지 않는다
- **frontmatter 안의 `#` 도 헤딩이 아니다**
- 파일 수정은 `vault.process` 로 하므로 읽고 쓰는 사이에 다른 편집이 끼어들지 않는다
- 🆔 를 켜면 볼트 전역에서 안 쓰인 6자리를 붙인다 (task 가 있는 파일만 훑는다)
- `#gcal/…` 는 **태그 바로 뒤, 내용 앞**에 붙인다. tasks-gcal-sync 가 이벤트 제목에서 이 태그를 떼므로 캘린더에는 내용만 남는다
- GCal 캘린더는 목록에 없는 이름도 그대로 쓸 수 있다 (자동완성은 편의용). 없는 캘린더면 tasks-gcal-sync 가 기본 캘린더로 보낸다
- 시작일이 마감일보다 뒤면 거부한다
- 잘못된 규칙은 그 규칙만 건너뛰고 블록에 오류 박스를 띄운다 — 조용히 실패하지 않는다

## 요구 플러그인

없다. Tasks 플러그인이 있으면 넣은 줄이 쿼리·캘린더에 잡히지만, 이 플러그인 자체는
Dataview / QuickAdd / Templater 없이 돈다.

## 설치 (BRAT)

1. BRAT 설치 → `Add Beta plugin`
2. `hjmoon9212/quick-add-button` 입력
3. 설정에서 Quick Add Button 활성화

## 개발

```bash
npm install
npm test               # 삽입 위치(findInsertIndex) · 경로 해석(paths) 테스트
npm run build          # tsc 타입체크 + 프로덕션 번들

# 볼트로 바로 감시 빌드
OBSIDIAN_PLUGIN_DIR=C:/obsidian/ob_Moon/.obsidian/plugins/quick-add-button npm run dev
```

배포는 태그 push 로만 한다 (볼트의 `main.js` 를 직접 고치지 않는다).

```bash
node version-bump.mjs 0.2.0
git commit -am "v0.2.0" && git push
git tag 0.2.0 && git push origin 0.2.0   # v 접두사 없음, manifest.version 과 동일
```

태그가 올라가면 GitHub Actions 가 빌드해서 `main.js` / `manifest.json` /
`styles.css` / `versions.json` 을 첨부한 Release 를 만들고, BRAT 가 그걸 받아간다.

## 라이선스

MIT
